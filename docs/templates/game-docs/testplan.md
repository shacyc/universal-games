...
| --- | --- | --- | --- |
| S0 | Gates and invariants | `pnpm game:check <slug>` | todo |
...
| S8 | Blast radius | each commit touches only `games/<slug>/`, `catalog.json`, one `demoData.ts` line; the hook refuses a game commit that also edits `packages/sdk/` | todo |
