...
Copy the four document templates from `docs/templates/game-docs/` —
`brief.md`, `plan.md`, `testplan.md`, `progress.md` — into `games/<slug>/docs/`
(`art-assets.json` there is optional and copied later, only if the game needs
bitmap art — §8). Then fill `brief.md` **by interviewing the owner**. How to
...
13. **Image files are generated from an art spec, never sourced from the web.**
    A texture, a background, a sprite sheet, an illustration on the title card:
    describe it in `games/<slug>/docs/art-assets.json` (§8, *Art*), hand that
    spec to an image-generating agent, and commit what it writes under
    `games/<slug>/public/`. Never download art, hotlink a CDN, or ship a
    `TODO: art` placeholder — an installed game plays offline, so anything the
    build does not contain is a blank rectangle on a phone in a tunnel, and the
    licence on a downloaded asset is nobody's job to untangle a year later. Two
    things stay hand-authored, because a generator is the wrong tool for them:
    ...
    baked into a bitmap is a user-facing string rule 11 cannot reach and no
...
games/<slug>/
  ...
  docs/art-assets.json          OPTIONAL — the bitmap-art spec, only if the game needs one (§8, Art)
...
Rule 13: bitmap art is **specified in the game, generated by an image agent**.
You do not run an image model yourself and no particular tool is assumed. You
write the spec; whoever has image-generation available runs it against that
spec and commits the files.

**If the game needs no bitmap at all** — the normal case for a puzzle or a board
game — write no spec and generate nothing. CSS and canvas drawing are smaller,
sharper and translatable.

**If it does**, add a fifth doc, `games/<slug>/docs/art-assets.json`, a
machine-readable run-list. Copy `docs/templates/game-docs/art-assets.json` and
fill it in. Each entry in `assets[]` carries:
...
| `postProcess`, `acceptCheck` | any cleanup step, and what a human confirms by eye before committing |

Top-level `globalConstraints` and `afterGenerating` hold the rules that apply to
every image (no text, flat style, small files) and the steps to run once the
files exist (eyeball for glyphs, `pnpm build`, confirm `dist/g/<slug>/art/`,
commit).

`prefer .webp` for everything — it carries alpha and stays small.

**Handing it off.** Give the JSON plus
`docs/templates/game-docs/art-assets.prompt.md` to an image-generating agent;
that prompt is generic and drives the whole run from the JSON alone. Until the
files land, `src/assets.ts` must load each one and **fall back to procedural
canvas drawing when it is missing** (`img.onerror → null`), so the game is fully
playable before any art exists and dropping the `.webp` files in plus a rebuild
switches them on with no code change. `public/icon.svg` and the catalog `cover`
are never in the spec — they stay hand-authored vector.
