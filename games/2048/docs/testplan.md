...
| M20 | One choice, every surface | hub in one tab, standalone `/g/2048/` in another; change language in each | the other follows live, no reload, in both directions | pass — 2026-09-09. Hub → game and game → hub both propagated; `performance.getEntriesByType('navigation').length` stayed 1 on the receiving side, and scores re-formatted (`1.024` ⇄ `1,024`) |
