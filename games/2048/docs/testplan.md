...
| M18 | Settings, standalone | same, at `/g/2048/` with no shell | identical behaviour; "back to home" lands on `/` | pass — 2026-09-09, re-run after the language moved onto the user record. `window.parent === window`, the change landed on the user record (`locale: "en"`), and the exit navigated to the hub, which came up in the same language |
...
| M20 | The language rides on the user | change it in the game, wipe nothing, reload the hub | the hub opens in it; `localStorage` holds no language key; the user record does | pass — 2026-09-09. After picking English in the game, the record read `{ isAnonymous: true, locale: "en" }`, `localStorage` held only `arcade:theme`, and the hub reloaded into English with EN pressed |
| M21 | Cross-tab, same moment | hub in one tab, standalone game in another; change language in one | the other follows **on its next load**, not live | **not recorded** — live cross-tab propagation was deliberately dropped with `localStorage` (decision 20). Worth confirming the next-load behaviour on a device |
