...
one object per image with `prompt`, `path`, `file`, chroma-key notes and accept
checks. Hand that file, plus
[`docs/templates/game-docs/art-assets.prompt.md`](../../../docs/templates/game-docs/art-assets.prompt.md),
to an image-generating agent. This page is the human context.

## Rules

- Bitmap art is **specified here and generated by an image agent** (rule 13),
  never sourced from the web. No particular image tool is assumed — the JSON
  plus the generic prompt drives the whole run.
- **No text, letters or numbers** in any generated image — a word baked into a
  bitmap is a string the i18n layer cannot reach.
...
- Image models paint a background, not an alpha channel. Sprites that need
