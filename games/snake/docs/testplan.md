...
| 8 | first taper attempt was a chain of discs — the whole body looked lumpy / "bubbly" (owner: "toàn thân tròn tròn nhìn kì cục") | overlapping discs of varying radius scallop the outline | `render.ts` — body is a single constant-width **stroke**; the last ≤5 cells are a **filled ribbon** whose half-width eases (smoothstep) `0.41·cell → 0` to a point; a disc hides the stroke↔ribbon seam; head blob `1.2×` body width. Added a flicking red forked **tongue** (`drawTongue`, cosmetic, hidden while dead or chomping; static under reduced-motion) | device (visual) |
