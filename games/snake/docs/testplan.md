...
| 7 | on a turn the body cut a **diagonal** across the corner and segments seemed to shimmer/overlap | `bodyPoints()` slid every interior vertex each tick, so mid-turn two adjacent vertices were offset on both axes and the stroked polyline between them ran diagonally; the round line-joins slid too | `render.ts` — interior vertices now stay *exactly* on cell centres (clean right angles); only the head slides out of the neck and the tail retracts. `drawSnakeBody()` no longer strokes a polyline — it fills a dense chain of overlapping discs (one `fill()`, no seams), so corners read as smooth round bends | device (feel) |
| 8 | tail was a blunt round stub, not snake-like | `drawSnakeBody()` used one constant `lineWidth` | `render.ts` — disc radius eases (smoothstep) from `0.41·cell` down to `0.14·cell` over the last 5 points; head disc is `1.08×` for a fuller blob | device (visual) |
