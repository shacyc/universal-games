...
| U27 | Wall-grace predicate | `willHitWall(run)` — heading off the board; safe heading; a queued turn that clears the wall; a queued reversal; a dead run | `true` only when the next head cell (same turn-resolution as `step`) is off-grid; a real escaping turn flips it `false`; a reversal is ignored; dead → `false` | `willHitWall: *` (5 cases) | pass |
...
| 5 | input felt laggy — up to a full tick (~167 ms at start speed) between a key press and the snake turning, plus a dead pause after the very first input | the tick accumulator started at 0, so the first `step` was always one `tickMs` away, and mid-run turns only ever applied on the next tick boundary | `main.ts` — prime `acc = tickMs()` on run / countdown start; a turn pulls the next tick forward to within `TURN_LAT_MS` (55 ms), clamped so it never lands sooner than `tickMs − TURN_LAT_MS` after the last tick (no free step, average speed unchanged) | device (feel) |
| 6 | eating an apple made the tail "kick back" a cell — a visible jerk | `bodyPoints()` interpolated only the head and the tail; on a growth tick the tail branch was skipped, so every middle segment snapped a cell forward while the previous frame had left the tail interpolated a cell inward | `render.ts` — see #7; `bodyPoints()` reworked so the tail interpolates `prevTailCell → curTailCell` (identical on a growth tick → holds still, no lurch) | device (feel) |
| 7 | on a turn the body cut a **diagonal** across the corner and segments seemed to shimmer/overlap | `bodyPoints()` slid every interior vertex each tick, so mid-turn two adjacent vertices were offset on both axes and the polyline between them ran diagonally; the round line-joins slid too | `render.ts` — interior vertices now stay *exactly* on cell centres (clean right angles); only the head slides out of the neck and the tail retracts. `drawSnakeBody()` **strokes** the full-width run (round `lineJoin`/`lineCap` → smooth round corners) and fills a short ribbon only for the taper | device (feel) |
| 8 | disc-chain taper made the whole body look lumpy / "bubbly" (owner: "toàn thân tròn tròn nhìn kì cục"); then the ribbon taper was dropped too (owner: "bỏ yêu cầu vuốt đuôi, giữ ổn định thân") | overlapping discs of varying radius scallop the outline | `render.ts` — body is **one constant-width stroke** end to end (round `lineJoin`/`lineCap`), plus a head blob `1.2×` body width. No taper. Added a flicking red forked **tongue** (`drawTongue`, cosmetic; hidden while dead or chomping; static under reduced-motion) | device (visual) |
| 9 | no reaction window before a wall — a swipe a frame too late still died | `step` killed on the fatal tick with no delay | `main.ts` — if `willHitWall(run)` the tick is held `WALL_GRACE_MS` (50 ms): `acc` pinned at `tickMs − 1` (snake hangs at the brink), re-checked each frame, so a turn queued in the window escapes instead. `snake.ts` `willHitWall` (U27). Walls only | device (feel) |
